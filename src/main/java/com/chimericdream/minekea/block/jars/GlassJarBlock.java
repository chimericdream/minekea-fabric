...
public class GlassJarBlock extends Block implements MinekeaBlock, BlockEntityProvider {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (isFilledBucket(heldItem)) {
            Identifier itemId = Registry.ITEM.getId(heldItem.getItem());
            Fluid bucketFluid = getFluidType(itemId);
     ...
     */
    private Fluid getFluidType(Identifier itemId) {
        if (itemId.getPath().equals("water_bucket")) {
            return Fluids.WATER;
        }

        if (itemId.getPath().equals("lava_bucket")) {
            return Fluids.LAVA;
        }

        if (itemId.getPath().equals("milk_bucket")) {
            return com.chimericdream.minekea.fluid.Fluids.MILK;
        }

        if (itemId.getPath().equals(HoneyBucketItem.ITEM_ID.getPath())) {
    ...
    private boolean isFilledBucket(ItemStack item) {
        ...
        return !Registry.ITEM.getId(item.getItem()).getPath().equals("bucket");
    }
