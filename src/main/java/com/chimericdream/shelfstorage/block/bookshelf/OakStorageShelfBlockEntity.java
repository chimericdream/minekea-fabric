...
import com.chimericdream.shelfstorage.screen.OakStorageShelfScreenHandler;
...
public class OakStorageShelfBlockEntity extends BlockEntity implements NamedScreenHandlerFactory, ImplementedInventory {
    private final DefaultedList<ItemStack> items = DefaultedList.ofSize(9, ItemStack.EMPTY);

    public OakStorageShelfBlockEntity(BlockPos pos, BlockState state) {
    ...
    public ScreenHandler createMenu(int syncId, PlayerInventory playerInventory, PlayerEntity player) {
        return new OakStorageShelfScreenHandler(syncId, playerInventory, this);
    }

    public static void tick(World world, BlockPos pos, BlockState state, OakStorageShelfBlockEntity entity) {
        if (world.isClient()) {
            return;
        }
