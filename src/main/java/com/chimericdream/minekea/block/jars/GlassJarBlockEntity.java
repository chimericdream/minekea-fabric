...
public class GlassJarBlockEntity extends BlockEntity implements BlockEntityClientSerializable, ImplementedInventory {
    ...
    public boolean canAcceptItem(ItemStack item) {
        ...
        if (!MinekeaTags.GLASS_JAR_STORABLE.contains(item.getItem())) {
            return false;
        }
        ...
        return item.isItemEqual(storedItem) && (fullItemStacks < MAX_ITEM_STACKS || storedItem.getCount() < storedItem.getMaxCount());
    }
    ...
    public NbtCompound writeNbt(NbtCompound nbt) {
        ...
        if (storedFluid.matchesType(Fluids.EMPTY)) {
            ...
            Identifier fluidID = Registry.FLUID.getId(storedFluid);

            nbt.putString(FLUID_KEY, fluidID.toString());
