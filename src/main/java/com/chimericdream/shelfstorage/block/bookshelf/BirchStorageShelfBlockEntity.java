...
import com.chimericdream.shelfstorage.screen.BirchStorageShelfScreenHandler;
...
public class BirchStorageShelfBlockEntity extends BlockEntity implements NamedScreenHandlerFactory, ImplementedInventory {
    private final DefaultedList<ItemStack> items = DefaultedList.ofSize(9, ItemStack.EMPTY);

    public BirchStorageShelfBlockEntity(BlockPos pos, BlockState state) {
        super(Bookshelves.BIRCH_STORAGE_SHELF_BLOCK_ENTITY, pos, state);
    }
    ...
    public static void tick(World world, BlockPos pos, BlockState state, BirchStorageShelfBlockEntity entity) {
        if (world.isClient()) {
            return;
        }
